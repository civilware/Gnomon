...
func (g *GravitonStore) StoreLastIndexHeight(last_indexedheight int64, nocommit bool) (*graviton.Tree, bool, error) {
	...
	for g.migrating == 1 {
		logger.Printf("[StoreLastIndexHeight] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
	...
	if tree == nil {
		...
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) GetLastIndexHeight() int64 {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[GetLastIndexHeight] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	if v != nil {
		...
		if err != nil {
			logger.Printf("ERR - Error parsing stored int for lastindexheight: %v\n", err)
			return 0
		}
		return topoheight
	}
...
func (g *GravitonStore) StoreTxCount(count int64, txType string, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreTxCount] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) GetTxCount(txType string) int64 {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[GetTxCount] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	if v != nil {
		...
		if err != nil {
			logger.Printf("ERR - Error parsing stored int for txcount: %v\n", err)
			return 0
		}
		return txCount
	}

	//logger.Printf("[GetTxCount] No txcount stored for '%v'. Starting from 0\n", txType)
...
func (g *GravitonStore) StoreOwner(scid string, owner string, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreOwner] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) GetOwner(scid string) string {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[GetOwner] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) StoreNormalTxWithSCIDByAddr(addr string, normTxWithSCID *structures.NormalTXWithSCIDParse, nocommit bool) (*graviton.Tree, bool, error) {
	...
	for g.migrating == 1 {
		logger.Printf("[StoreSCIDInteractionHeight] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) StoreInvokeDetails(scid string, signer string, entrypoint string, topoheight int64, invokedetails *structures.SCTXParse, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreInvokeDetails] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-StoreInvokeDetails] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", scid)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
...
func (g *GravitonStore) GetAllSCIDInvokeDetails(scid string) (invokedetails []*structures.SCTXParse) {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetAllSCIDInvokeDetails] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", scid)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(scid)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) GetAllSCIDInvokeDetailsByEntrypoint(scid string, entrypoint string) (invokedetails []*structures.SCTXParse) {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetAllSCIDInvokeDetailsByEntrypoint] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", scid)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(scid)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) GetAllSCIDInvokeDetailsBySigner(scid string, signerPart string) (invokedetails []*structures.SCTXParse) {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetAllSCIDInvokeDetailsBySigner] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", scid)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(scid)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) StoreGetInfoDetails(getinfo *structures.GetInfo, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreGetInfoDetails] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) StoreSCIDVariableDetails(scid string, variables []*structures.SCIDVariable, topoheight int64, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreSCIDVariableDetails] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-StoreSCIDVariableDetails] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
...
func (g *GravitonStore) GetSCIDVariableDetailsAtTopoheight(scid string, topoheight int64) []*structures.SCIDVariable {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetSCIDVariableDetailsAtTopoheight] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(treename)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) GetAllSCIDVariableDetails(scid string) map[int64][]*structures.SCIDVariable {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetAllSCIDVariableDetails] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(treename)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) StoreSCIDInteractionHeight(scid string, height int64, nocommit bool) (*graviton.Tree, bool, error) {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-StoreSCIDInteractionHeight] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(treename)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) GetSCIDInteractionHeight(scid string) []int64 {
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-GetSCIDInteractionHeight] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(treename)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) StoreInvalidSCIDDeploys(scid string, fee uint64, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreInvalidSCIDDeploys] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	if tree == nil {
		var terr error
		logger.Printf("[Graviton-StoreInvalidSCIDDeploys] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", treename)
		prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
		tree, terr = prevss.GetTree(treename)
		if tree == nil {
			logger.Printf("[Graviton] ERROR: %v\n", terr)
...
func (g *GravitonStore) StoreMiniblockDetailsByHash(blid string, mbldetails []*structures.MBLInfo, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreMiniblockDetailsByHash] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) GetMiniblockDetailsByHash(blid string) []*structures.MBLInfo {
	...
	// Swap DB at g.DBMaxSnapshot+ commits. Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[GetMiniblockDetailsByHash] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) StoreMiniblockCountByAddress(addr string, nocommit bool) (*graviton.Tree, bool, error) {
	...
	// Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreMiniblockCountByAddress] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		store = g.DB
		ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
...
func (g *GravitonStore) StoreAltDBInput(treenames []string, altdb *GravitonStore) (err error) {
	...
	// Check for g.migrating, if so sleep for g.DBMigrateWait ms
	for g.migrating == 1 {
		logger.Printf("[StoreAltDBInput] G is migrating... sleeping for %v...\n", g.DBMigrateWait)
		time.Sleep(g.DBMigrateWait)
		//store = g.DB
		//ss, _ = store.LoadSnapshot(0) // load most recent snapshot
	}
	...
	for _, v := range treenames {
		...
		if tree == nil {
			var terr error
			logger.Printf("[Graviton-StoreAltDBInput] ERROR: Tree is nil for '%v'. Attempting to rollback 1 snapshot\n", v)
			prevss, _ := store.LoadSnapshot(ss.GetVersion() - 1)
